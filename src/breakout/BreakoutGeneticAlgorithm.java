...
public class BreakoutGeneticAlgorithm {
	...
	private BreakoutNeuralNetwork search() {
		...
		int eliteCount = (int) (POPULATION_SIZE * Commons.SELECTION_PERCENTAGE);
		...
		for (int i = 0; i < NUM_GENERATIONS; i++) {
			...
			MUTATION_PERCENTAGE = Math.min(0.2 * Commons.INITIAL_MUTATION_PERCENTAGE * noEvolutionInterval, 0.7);
	...
	private BreakoutNeuralNetwork mutate(BreakoutNeuralNetwork individual) {
		...
		if (Math.random() < Commons.MUTATION_RATE) {
